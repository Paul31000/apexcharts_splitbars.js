...
/**
 * ApexCharts BarSplitted Class responsible for drawing both Stacked Columns and Bars.
 *
 * @module BarSplitted
...
class BarSplitted extends Bar {
  ...
  draw(series, seriesIndex) {
    ...
    for (let i = 0, bc = 0; i < series.length; i++, bc++) {
      ...
      //odr
      this.lastBarSerie+=this.barInterval
      console.log("lastBarSerie")
      console.log(this.lastBarSerie)
  ...
  //odr
  returnSpacing(series) {
    let valeurMax = 0;

    // Parcourir chaque tableau dans le tableau principal
    for (let sousTableau of series) {
        // Parcourir chaque élément dans le sous-tableau
        for (let valeur of sousTableau) {
  ...
  drawStackedBarPaths({
    ...
    if (gsi > 0) {
      //odr ce qui nous intéresse!!!
      let bXP = zeroW
      //todo:trouver le max du graphe
      //todo:espacer la serie du max
